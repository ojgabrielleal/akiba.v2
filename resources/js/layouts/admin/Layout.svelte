    ...
    {#if alertMessage && alertColor}
        {#if Array.isArray(alertMessage)}
            {#each alertMessage as message}
                <div class={`mb-5 p-4 flex items-center gap-2 rounded-lg font-noto-sans font-light text-neutral-aurora transition-opacity duration-500 ease-in-out ${message ? "opacity-100" : "opacity-0"}`} style={`background-color: ${alertColor}`}>
