    ...
    <Section {title}>
        <form onsubmit={onSubmit}>
            ...
            <div class="mb-8">
                ...
                <input type="text" id="phrase" name="phrase" class="w-full h-[3rem] bg-neutral-aurora font-noto-sans rounded-lg outline-none pl-4" bind:value={$form.phrase}/>
            </div>
            <div class="mb-15">
...
{#if verify.onair === true && verify.streamer === true}
    <div class="flex justify-center mb-8">
        <button onclick={() => router.post("/painel/locucao/broadcast/end/")} type="submit" aria-label="iniciar programa" class="cursor-pointer w-full lg:w-auto py-2 px-6 border-4 border-solid border-red-crimson rounded-xl text-red-crimson text-xl font-bold font-noto-sans italic uppercase">
            Encerrar programa
        </button>
    </div>
{/if}
