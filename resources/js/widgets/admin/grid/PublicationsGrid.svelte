    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {#if publications?.data?.length > 0}
            {#each publications.data as item}
                <article class="w-full h-[14rem] lg:w-[18.2rem] flex-shrink-0 rounded-lg p-4 relative" style="background-color: {item.styles.bg ?? 'var(--color-blue-skywave)'}">
                    ...
                    <dl class="flex justify-between gap-5 absolute bottom-2 left-4 w-[calc(100%-2rem)]">
                        ...
                        <dd class="flex gap-3">
                            <a href={`${view}${item.slug}`} aria-label="Visualizar" class="cursor-pointer">
                                <img src="/icons/default/eye.svg" alt="" aria-hidden="true" class="w-5 filter-neutral-aurora"/>
                            </a>
                            {#if item.editable}
                                <a href={`${edit}${item.slug}`} aria-label="Editar" class="cursor-pointer">
                                    <img src="/icons/default/edit.svg" alt="" aria-hidden="true" class="w-4 filter-neutral-aurora"/>
                                </a>
                            {/if}
                        </dd>
                    </dl>
        ...
        {:else}
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <article class="w-full h-[14rem] lg:w-[18.2rem] flex-shrink-0 rounded-lg p-4 relative bg-blue-cerulean opacity-50">
                    <div class="font-noto-sans text-lg text-neutral-aurora line-clamp-5 uppercase">
