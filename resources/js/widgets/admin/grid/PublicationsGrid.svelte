    ...
    <div class="flex gap-5 overflow-x-auto flex-wrap">
        {#if publications?.data?.length > 0}
            {#each publications.data as item}
                <div
                    class="w-full h-[14rem] lg:w-[18.2rem] flex-shrink-0 rounded-lg p-4 relative"
                    style="background-color: {item?.styles?.bg ??
                        'var(--color-blue-skywave)'}"
                >
                    <span
                        class="font-noto-sans text-lg text-neutral-aurora line-clamp-5 uppercase"
                    >
                        {item.title}
                    </span>
                    <div
                        class="flex justify-between gap-5 absolute bottom-2 left-4 w-[calc(100%-2rem)]"
                    >
                        <span
                            class="font-noto-sans font-bold italic uppercase text-lg text-neutral-aurora"
                        >
                            {item?.user?.nickname}
                        </span>
        ...
        {:else}
            <div
                class="w-full h-[14rem] lg:w-[18.2rem] flex-shrink-0 rounded-lg p-4 relative bg-blue-cerulean opacity-50"
            >
                <span
                    class="font-noto-sans text-lg text-neutral-aurora line-clamp-5 uppercase"
                >
                    Humano, conteúdo? Nani sore? Os preguiçosos da Akiba estão
                    relaxando por aqui~ (｡♥‿♥｡) 🌸🍵💤
                </span>
                <div
                    class="flex justify-between gap-5 absolute bottom-2 left-4 w-[calc(100%-2rem)]"
                >
        ...
        {#if publications.last_page > 1}
            <div class="flex gap-5 mt-6">
                {#if publications.current_page > 1}
                    <button
                        on:click={() =>
                            pagination(publications.current_page - 1)}
