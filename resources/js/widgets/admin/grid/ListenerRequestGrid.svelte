    ...
    <Section {title}>
        <div class="flex justify-center">
            {#if verify.listener_request}
                <button onclick={()=>router.patch("/painel/locucao/listenerrequeststatus")} aria-label="parar de receber" class="cursor-pointer w-full lg:w-auto py-2 px-6 border-4 border-solid border-neutral-honeycream rounded-xl text-neutral-honeycream text-xl font-bold font-noto-sans italic uppercase">
                    Parar de receber
                </button>
            {:else}
                <button onclick={()=>router.patch("/painel/locucao/listenerrequeststatus")}  aria-label="começar a receber" class="cursor-pointer w-full lg:w-auto py-2 px-6 border-4 border-solid border-green-forest rounded-xl text-green-forest text-xl font-bold font-noto-sans italic uppercase">
                    Começar a receber
                </button>
            {/if}
        </div>
