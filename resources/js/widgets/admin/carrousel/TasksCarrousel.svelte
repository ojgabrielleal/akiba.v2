            ...
            {#each tasks as item}
                <div class="w-100 h-50 lg:w-[40rem] lg:h-43 flex-shrink-0 rounded-lg p-4 relative" style="background-color: {item.styles.bg};">        
                    ...
                    <div class="absolute top-5 right-5 rounded-xl shadow-lg w-[7rem] text-center overflow-hidden">
                        ...
                        <div class="text-neutral-aurora font-noto-sans italic font-extrabold text-2xl py-1 tracking-widest" style="background-color: {item.styles.bg_date.date}; color: {item.styles.bg_date.date_text_color}">
                            {item.deadline}
                        </div>
                    </div>
                    {#if item.due_soon}
