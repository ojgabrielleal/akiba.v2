        ...
        {#if tasks.length > 0}
            {#each tasks as item}
                <article class="w-100 h-50 lg:w-[40rem] lg:h-43 shrink-0 rounded-lg p-4 relative" style="background-color: {item.styles.bg};" >
                    ...
                    {#if item.due_soon}
                        <button type="button" class="font-noto-sans italic font-bold bg-red-crimson rounded-xl text-neutral-aurora uppercase absolute right-5 bottom-3 py-2 px-6 cursor-pointer" on:click={() => setTaskCompleted(item.id)}>
                            Solicitar Conclusão
                        </button>
                    {:else}
