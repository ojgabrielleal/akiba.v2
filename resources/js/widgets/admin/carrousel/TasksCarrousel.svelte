    ...
    <div class="scroll-x flex gap-5 overflow-x-auto flex-nowrap" bind:this={container} on:wheel={(e) => scrollx(e, container)} role="group">
                ...
                {#each tasks as item}
                    <div class="w-100 h-50 lg:w-[40rem] lg:h-43 flex-shrink-0 rounded-lg p-4 relative" style="background-color: {item.styles.bg};">        
                        ...
                        <div class="absolute top-5 right-5 rounded-xl shadow-lg w-[7rem] text-center overflow-hidden bg-neutral-aurora">
                            <div class="font-noto-sans italic font-bold text-sm py-1 tracking-wide uppercase" style="background-color: {item.styles.bg_date.title}; color: {item.styles.bg_date.title_text_color}">
                                Data Limite
                            </div>
                            <div class="font-noto-sans italic font-extrabold text-2xl py-1 tracking-widest" style="background-color: {item.styles.bg_date.date}; color: {item.styles.bg_date.date_text_color}">
                                {item.deadline}
                            </div>
                        </div>
                        {#if item.due_soon}
            ...
            {:else}
                <div class="w-100 h-50 lg:w-[40rem] lg:h-43 flex-shrink-0 rounded-lg p-4 relative bg-blue-cerulean opacity-50">        
                    ...
                    <span class="w-90 ont-noto-sans text-sm text-neutral-aurora line-clamp-5 mt-1">
                        Senpai~ no momento não tem task, então relaxa e vamos curtir anime juntos~ kawaii ne~ (｡•́︿•̀｡) 🌸🍵  
                        Pode ficar tranquilo, eu vou avisar quando tiver algo pra fazer yo~ (⌒‿⌒)✨
                    </span>
                    <div class="absolute top-5 right-5 rounded-xl shadow-lg w-[7rem] text-center overflow-hidden bg-neutral-aurora">
                        <div class="font-noto-sans italic font-bold text-sm py-1 tracking-wide uppercase bg-blue-indigo text-neutral-aurora">
                            Data Limite
                        </div>
                        <div class="font-noto-sans italic font-extrabold text-2xl text-blue-indigo py-1 tracking-widest">
                            00/00
                        </div>
                    </div>
                </div>
